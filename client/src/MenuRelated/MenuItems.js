...
const MenuItems = () => {
  ...
  useEffect(() => {
    if (id === "regular") {
      setFoods(
        RegularFoods.categories.find((cat) => cat.name === category).foods
      );
      setCategoryInfo(
        RegularFoods.categories.find((cat) => cat.name === category).filters
      );
    } else if (id === "deluxe") {
      setFoods(
        DeluxeFoods.categories.find((cat) => cat.name === category).foods
      );
      setCategoryInfo(
        DeluxeFoods.categories.find((cat) => cat.name === category).filters
      );
    } else {
    }
  }, []);
        ...
        >
          {categoryInfo.map((filter, index) => (
            <Grid key={index} xs={2} sm={4} md={4}>
              <FilterChip
                label={`${filter} (${numOfFilteredFoods(filter)})`}
                onClick={() => {
                  handleChipToggle(filter);
                }}
